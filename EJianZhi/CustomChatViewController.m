...
@interface CustomChatViewController ()<UITableViewDelegate,UITableViewDataSource, UISearchDisplayDelegate,SRRefreshDelegate, UISearchBarDelegate, IChatManagerDelegate>
...
#pragma mark - private By GYB

//筛选函数
-(NSArray *)filterArray:(NSArray *)origin byKeyWord:(NSString*)keyword
{
    NSArray *filteredConversation=[NSArray array];
    //注意效率不是很高
    for (EMConversation *obj in origin) {
        //            NSString *Id=obj.chatter;
        //            NSLog(@"会话Id:%@",Id);
        NSLog(@"chaterid:%@",obj.chatter);
        NSLog(@"chaterid:%@",[obj.chatter substringToIndex:3]);
        if([[obj.chatter substringToIndex:3] isEqualToString:keyword])
        {
            filteredConversation=[filteredConversation arrayByAddingObject:obj];
        }
    }
...
//有条件的获取列表
- (NSMutableArray *)loadDataSourceByCategory:(NSString*)keyWord
{
    NSMutableArray *ret = nil;
    NSArray *conversations = [[EaseMob sharedInstance].chatManager conversations];
    NSArray *filteredConversation=
    [self filterArray:conversations byKeyWord:keyWord];
    
    NSArray* sorte = [filteredConversation sortedArrayUsingComparator:
...
//    self.dataSource = [self loadDataSource];
    if (self.categoryKeyWord!=nil) {
        self.dataSource=[self loadDataSourceByCategory:self.categoryKeyWord];
    }
    else {
        self.dataSource = [self loadDataSource];
    }
